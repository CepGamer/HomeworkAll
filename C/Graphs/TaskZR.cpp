...
bool substr() {
	...
	while (i < n) {
		if (cur == -1)
			return false;
		for (int j = 0; (j < t[cur].len()) && (i < n); j++)
			if (sub[i++] != s[t[cur].l + j])
				return false;
		cur = t[cur].next[sub[i] - 'a'];
	}
	return true;
}
...
int main()
{
	...
	n = 0;
	in = fopen("dictionary.in", "r");
	out = fopen("dictionary.out", "w");
	...
	n = 0;
	for (int i = 0; i < k; i++) {
		while (fscanf(in, "%c", &sub[n]), sub[n++] >= 32);
		n--;
		fprintf(out, substr() ? "Yes\n" : "No\n");
		n = 0;
	}

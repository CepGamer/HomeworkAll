...
mydivpol as bs = mydiv' (mynorm as) (mynorm bs) []
  where mydiv' a s q | ddif < 0 = (q, a)
            ...
            where   ddif = (mydeg a) - (mydeg s)
                    ...
                    ks = mymap (mymult k) $ myshift ddif s
                    q' = myzip' (+) q $ myshift ddif [k]
                    a' = mynorm $ tail $ myzip' (-) a ks

myprint' (a, b) = if b == 1 then show a else "(" ++ show a ++ "/" ++ show b ++ ")"
myprint [] = ""
myprint ((a, b):as) = (myprint' (a, b) ++ "x" ++ (if mydeg as == 0 then "" else "^(" ++ show (mydeg as + 1) ++ ")")) ++ "+" ++ myprint as
